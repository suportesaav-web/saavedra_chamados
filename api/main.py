...
# Caminho absoluto para os uploads no Windows
...
# ==========================================
# 3. FUNÇÕES AUXILIARES E MOTOR DE E-MAIL
# ==========================================
...
def enviar_email_abertura(destinatario: str, nome_usuario: str, tarefa_id: int, titulo: str):
    corpo_html = f"""
    <div style="font-family: 'Segoe UI', Tahoma, sans-serif; color: #25282a; max-width: 600px; margin: 0 auto; border: 1px solid #eaeaea; border-top: 5px solid #dc4405; border-radius: 8px; box-shadow: 0 4px 15px rgba(0,0,0,0.05);">
        ...
        <div style="padding: 30px; background: #ffffff;">
            ...
            <p style="font-size: 14px; color: #555;">O seu chamado foi registado com sucesso na nossa fila de atendimento técnico e já está a ser processado.</p>
...
def enviar_email_atualizacao(destinatario: str, nome_usuario: str, tarefa_id: int, status_nome: str, comentario: str, status_id: int):
    ...
    if status_id == 4:
        ...
        <div style="background: #f0f4f8; padding: 20px; border-radius: 6px; margin-top: 20px; text-align: center;">
            ...
            <div style="display: flex; justify-content: center; gap: 8px;">
                ...
                <a href="http://10.0.0.252:8082/detalhe_chamado.html?id={tarefa_id}&avaliar=2" style="background: #dc4405; color: white; padding: 8px 12px; text-decoration: none; border-radius: 4px; font-weight: bold; font-size: 12px;">2 😕</a>
                <a href="http://10.0.0.252:8082/detalhe_chamado.html?id={tarefa_id}&avaliar=3" style="background: #f0ad4e; color: white; padding: 8px 12px; text-decoration: none; border-radius: 4px; font-weight: bold; font-size: 12px;">3 😐</a>
                <a href="http://10.0.0.252:8082/detalhe_chamado.html?id={tarefa_id}&avaliar=4" style="background: #5cb85c; color: white; padding: 8px 12px; text-decoration: none; border-radius: 4px; font-weight: bold; font-size: 12px;">4 😊</a>
    ...
    corpo_html = f"""
    <div style="font-family: 'Segoe UI', Tahoma, sans-serif; color: #25282a; max-width: 600px; margin: 0 auto; border: 1px solid #eaeaea; border-top: 5px solid {cor_topo}; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 15px rgba(0,0,0,0.05);">
        <div style="background: #25282a; padding: 20px; text-align: center;">
            <h2 style="margin: 0; color: #ffffff; font-size: 20px;">Saavedra <span style="color: #dc4405;">Chamados</span></h2>
        </div>
        <div style="padding: 30px; background: #ffffff;">
            ...
            <div style="background: #f8f9fa; padding: 20px; border-radius: 6px; margin: 20px 0; border: 1px solid #eaeaea;">
                <div style="margin-bottom: 12px;">
                    <span style="font-size: 12px; color: #888;">Estado Atual:</span><br>
                    <span style="background: {cor_topo}; color: white; padding: 4px 10px; border-radius: 12px; font-size: 12px; font-weight: bold;">{status_nome}</span>
                </div>
                <div>
                    <span style="font-size: 12px; color: #888;">Nota da Equipa Técnica:</span>
                    <p style="margin: 0; font-size: 14px; background: #ffffff; padding: 12px; border-radius: 4px; border: 1px solid #e0e0e0; white-space: pre-wrap;">{comentario}</p>
                </div>
            </div>
            {bloco_csat}
            <div style="text-align: center; margin-top: 30px;">
                <a href="http://10.0.0.252:8082/detalhe_chamado.html?id={tarefa_id}" style="display: inline-block; background: {cor_topo}; color: #ffffff; padding: 12px 25px; text-decoration: none; border-radius: 4px; font-weight: bold;">Ver Detalhes</a>
            </div>
        </div>
    </div>
    """
...
# ==========================================
# 5. ROTAS GERAIS (AUTH E CADASTROS)
# ==========================================
@app.post('/api/auth/login')
def login_local(request: Request, login_data: LoginRequest):
    with engine.connect() as conn:
        user = conn.execute(text("SELECT USUARIO_ID, NOME, EMAIL, PERFIL, SENHA_HASH FROM tbUSUARIO WHERE LOWER(EMAIL) = :email AND ATIVO = 1"), {"email": login_data.email.strip().lower()}).fetchone()
...
def get_cadastros(tipo_cadastro: str, usuario: dict = Depends(get_usuario_sessao)):
    ...
    with engine.connect() as conn: return [{"id": r[0], "nome": r[1]} for r in conn.execute(text(f"SELECT {cfg['id']}, {cfg['nome']} FROM {cfg['tabela']} WHERE ATIVO = 1"))]
...
def delete_cadastro(tipo_cadastro: str, id_registro: int, usuario: dict = Depends(exigir_admin)):
    cfg = TABELAS_PERMITIDAS[tipo_cadastro]
    with engine.begin() as conn: conn.execute(text(f"UPDATE {cfg['tabela']} SET ATIVO = 0 WHERE {cfg['id']} = :id"), {"id": id_registro})
    return {"message": "Inativado"}

@app.get("/api/usuarios")
def get_usuarios(usuario: dict = Depends(get_usuario_sessao)):
    with engine.connect() as conn:
        return [{"id": r[0], "nome": r[1], "email": r[2], "ad_login": r[3], "perfil": r[4], "setor": r[5], "setor_id": r[6], "nivel_acesso": r[7]} for r in conn.execute(text("SELECT U.USUARIO_ID, U.NOME, U.EMAIL, U.AD_LOGIN, U.PERFIL, S.SETOR_NOME, U.SETOR_ID, U.NIVEL_ACESSO FROM tbUSUARIO U LEFT JOIN tbSETOR S ON U.SETOR_ID = S.SETOR_ID WHERE U.ATIVO = 1"))]

@app.post("/api/usuarios")
def create_usuario(u: UsuarioCreate, usuario: dict = Depends(exigir_admin)):
    with engine.begin() as conn: conn.execute(text("INSERT INTO tbUSUARIO (NOME, EMAIL, AD_LOGIN, SETOR_ID, PERFIL, NIVEL_ACESSO, SENHA_HASH, ATIVO) VALUES (:n, :e, :a, :s, :p, :na, :senha, 1)"), {"n": u.nome, "e": u.email, "a": u.ad_login, "s": u.setor_id, "p": u.perfil, "na": u.nivel_acesso, "senha": hash_senha(u.senha if u.senha else "saavedra123")})
    return {"message": "Criado"}

@app.put("/api/usuarios/{id}")
def update_usuario(id: int, u: UsuarioUpdate, usuario: dict = Depends(exigir_admin)):
    with engine.begin() as conn: conn.execute(text("UPDATE tbUSUARIO SET NOME=:n, EMAIL=:e, AD_LOGIN=:a, SETOR_ID=:s, PERFIL=:p, NIVEL_ACESSO=:na WHERE USUARIO_ID=:id AND ATIVO = 1"), {"id": id, "n": u.nome, "e": u.email, "a": u.ad_login, "s": u.setor_id, "p": u.perfil, "na": u.nivel_acesso})
    return {"message": "Atualizado"}

@app.delete("/api/usuarios/{id}")
def delete_usuario(id: int, usuario: dict = Depends(exigir_admin)):
    with engine.begin() as conn: conn.execute(text("UPDATE tbUSUARIO SET ATIVO = 0 WHERE USUARIO_ID = :id"), {"id": id})
    return {"message": "Desativado"}

@app.get("/api/admin/sla-matrix")
def get_sla_matrix(usuario: dict = Depends(exigir_admin)):
    with engine.connect() as conn: return [{"id": r[0], "prioridade": r[1], "tipo": r[2], "tempo_horas": r[3], "prioridade_id": r[4], "tipo_id": r[5]} for r in conn.execute(text("SELECT M.SLA_ID, P.PRIORIDADE_NOME, TP.TIPO_NOME, M.TEMPO_HORAS, M.PRIORIDADE_ID, M.TIPO_ID FROM tbSLA_CONFIG M INNER JOIN tbPRIORIDADE P ON M.PRIORIDADE_ID = P.PRIORIDADE_ID INNER JOIN tbTIPO TP ON M.TIPO_ID = TP.TIPO_ID WHERE M.ATIVO = 1 ORDER BY TP.TIPO_NOME, P.PRIORIDADE_ID ASC"))]
...
def get_kpis(visao_equipe: bool = False, usuario: dict = Depends(get_usuario_sessao)):
    ...
    return {"sla_estourado": sla_estourado, "sla_atencao": sla_atencao, "criticos": criticos, "aguardando_triagem": triagem, "status_dinamicos": [{"id": r[0], "nome": r[1], "qtd": r[2]} for r in res_status]}
...
# ==========================================
# 7. MOTOR DE PROCESSAMENTO DE FILA
# ==========================================
def processar_fila_com_filtros(base_query: str, count_query: str, params: dict, status_id: Optional[int], prioridade_id: Optional[int], tipo_id: Optional[int], sla_filtro: Optional[str], data_inicio: Optional[str], data_fim: Optional[str], user_id_filtro: Optional[int] = None, tecnico_id_filtro: Optional[int] = None, sem_tecnico: bool = False):
    ...
    if sem_tecnico:
        where_conds.append("T.TECNICO_ID IS NULL")
    ...
    with engine.connect() as conn:
        ...
        # 🌟 ORDENAÇÃO SEGURA: Se for visão de equipe/técnico (user_id_filtro is None), prioriza ativos e prazos
        ...
        query_final = f"{base_query} {where_clause} {ordem} OFFSET :offset ROWS FETCH NEXT :limit ROWS ONLY"
        rows = conn.execute(text(query_final), params).fetchall()
        
    return {
        "dados": [{"id": r[0], "titulo": r[1], "status": r[2], "solicitante": r[3], "tecnico": r[4], "data_limite_sla": formatar_data_segura(r[5]), "status_id": r[6], "prioridade_id": r[7]} for r in rows], 
        "paginas": (total_items + params["limit"] - 1) // params["limit"]
    }
...
def get_tarefa_historico(tarefa_id: int, usuario: dict = Depends(get_usuario_sessao)):
    with engine.connect() as conn:
        ...
        if usuario.get("perfil") not in PERFIS_ADMIN:
            query += " AND (H.NOTA_INTERNA = 0 OR H.NOTA_INTERNA IS NULL)"
...
@app.get("/api/tarefas")
def get_tarefas(page: int = 1, limit: int = 20, data_inicio: Optional[str] = None, data_fim: Optional[str] = None, status_id: Optional[int] = None, prioridade_id: Optional[int] = None, tipo_id: Optional[int] = None, sla_filtro: Optional[str] = None, visao_equipe: bool = False, sem_tecnico: bool = False, meus_pessoais: bool = False, usuario: dict = Depends(exigir_admin)):
    base = "SELECT T.TAREFA_ID, T.TITULO, S.STATUS_NOME, U.NOME, TEC.NOME, T.DATA_LIMITE_SLA, T.STATUS_ID, T.PRIORIDADE_ID FROM tbTAREFAS T LEFT JOIN tbSTATUS S ON T.STATUS_ID = S.STATUS_ID LEFT JOIN tbUSUARIO U ON T.SOLICITANTE_ID = U.USUARIO_ID LEFT JOIN tbUSUARIO TEC ON T.TECNICO_ID = TEC.USUARIO_ID"
    
    if meus_pessoais:
        user_id_filtro = usuario["id"]
        tecnico_filtro = None
    else:
        user_id_filtro = None
        tecnico_filtro = None if (visao_equipe or sem_tecnico) else usuario["id"]

    return processar_fila_com_filtros(base, "SELECT COUNT(*) FROM tbTAREFAS T", {"offset": (page - 1) * limit, "limit": limit}, status_id, prioridade_id, tipo_id, sla_filtro, data_inicio, data_fim, user_id_filtro=user_id_filtro, tecnico_id_filtro=tecnico_filtro, sem_tecnico=sem_tecnico)
...
def create_tarefa(tarefa: TarefaCreate, background_tasks: BackgroundTasks, usuario: dict = Depends(get_usuario_sessao)):
    ...
    # Se for usuário comum, força que fique sem técnico (vai para a fila de triagem)
...
async def anexar_arquivo(tarefa_id: int, historico_id: Optional[int] = Form(None), file: UploadFile = File(...), usuario: dict = Depends(get_usuario_sessao)):
    with engine.connect() as conn: t = conn.execute(text("SELECT SOLICITANTE_ID FROM tbTAREFAS WHERE TAREFA_ID = :id"), {"id": tarefa_id}).fetchone()
    ...
    with engine.begin() as conn: conn.execute(text("INSERT INTO tbTAREFA_ANEXO (TAREFA_ID, HISTORICO_ID, NOME_ORIGINAL, NOME_SALVO, DATA_HORA) VALUES (:t_id, :h_id, :nome_orig, :nome_salvo, GETDATE())"), {"t_id": tarefa_id, "h_id": historico_id, "nome_orig": file.filename, "nome_salvo": nome_salvo})
    return {"status": "sucesso"}

# ==========================================
# 9. ROTA DE PESQUISA DE SATISFAÇÃO (CSAT)
# ==========================================
...
def avaliar_chamado(tarefa_id: int, nota: int, request: Request):
    ...
    texto_nota = desc_notas.get(nota, f"{nota}/5")
    
    with engine.begin() as conn:
        conn.execute(
            text("UPDATE tbTAREFAS SET NOTA_CSAT = :nota WHERE TAREFA_ID = :id"),
            {"nota": nota, "id": tarefa_id}
        )
        conn.execute(
            text("INSERT INTO tbTAREFA_HISTORICO (TAREFA_ID, USUARIO_ID, STATUS_ID_NA_OCASIAO, COMENTARIO, DATA_HORA, NOTA_INTERNA) "
                 "SELECT :id, SOLICITANTE_ID, STATUS_ID, :comentario, GETDATE(), 0 FROM tbTAREFAS WHERE TAREFA_ID = :id"),
            {"id": tarefa_id, "comentario": f"Nota de Atendimento atribuída pelo utilizador: {texto_nota}"}
        )
